...
void uart2_config (void)
{
	...
	//! [2] 3. Program the M bit in USART_CR1 to define the word length.
	//USART3->CR1 &= ~(USART_CR1_M0 | USART_CR1_M1);
	USART2->CR1 &= ~USART_CR1_M; 									// We set the M as 0 for 8 bit message format.

	//! [4] 4. Select the desired baud rate using the baud rate register USART_BRR.
	USART2->CR1 &= ~USART_CR1_OVER8;								// Set the over-sampling by 16 to be sure.
	// For 115200 baud rate the BRR calculation via the RM0401 s.24.4.4 formula. 0x139 = 313d
	// Mantissa id 0d27 := 0x1B
	// Fraction is 0d2 := 0x2
	...
	//! [1] 6. Enable the USART by writing the UE bit in USART_CR1 register to 1.
